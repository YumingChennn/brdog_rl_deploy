...
class Controller:
    def __init__(self):
        ...
        parser.add_argument("config_file", type=str, nargs='?', default="config/big_reddog_lab_v1.yaml", help="config file name in the config folder")
    ...
    def move(self):
        if self.counter % self.control_decimation == 0 and self.counter > 0:
            ...
            current_obs = np.concatenate(obs).astype(np.float32)
            obs_tensor = torch.from_numpy(current_obs).float().unsqueeze(0)  # Add batch dimension

            obs_tensor_buf = torch.clip(obs_tensor, -100, 100).unsqueeze(0)  # Add batch dimension and clip for safety
