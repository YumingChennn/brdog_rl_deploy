...
namespace
{
  ...
  struct SimulationConfig
  {
    ...
    int enable_angular_velocity_noise = 0;
    double angular_velocity_noise_range = 0.2;
          ...
          if (sim.run)
          {
            ...
            else
            {
                     ...
                     mj::Simulate::Clock::now() - startCPU < Seconds(refreshTime))
              {
                ...
                // add noise to angular velocity (qvel[3:6])
                if (config.enable_angular_velocity_noise)
                {
                  for (int i = 3; i < 6; i++)
                  {
                    // uniform random noise in [-range, +range]
                    double rand_uniform = (double)rand() / RAND_MAX; // [0, 1]
                    double noise = (2.0 * rand_uniform - 1.0) * config.angular_velocity_noise_range;
                    d->qvel[i] += noise;
                  }
                }
...
int main(int argc, char **argv)
{
  ...
  config.enable_angular_velocity_noise = yaml_node["enable_angular_velocity_noise"].as<int>();
  config.angular_velocity_noise_range = yaml_node["angular_velocity_noise_range"].as<double>();
